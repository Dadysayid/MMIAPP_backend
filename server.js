...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    if (
      ...
      cb(
        new Error("Type de fichier non supporté. Utilisez des images ou PDFs."),
        false
      );
    }
  },
...
const uploadMultiple = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    if (
      ...
      cb(
        new Error(
          "Type de fichier non supporté. Utilisez des images, PDFs, documents Office, ZIP ou texte."
        ),
        false
      );
    }
...
const uploadEauMineral = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    if (
      ...
      cb(
        new Error(
          "Type de fichier non supporté. Utilisez des documents professionnels standard."
        ),
        false
      );
    }
...
function mailActivationHTMLWithLoginInfo({
  nom,
  prenom,
  activationLink,
  role,
...
async function enregistrerSuivi(
  ...
  await conn.execute(
    ...
    [
      ...
      statutPrecedent,
      nouveauStatut,
      donneesSupplementaires ? JSON.stringify(donneesSupplementaires) : null,
    ]
  );
}
...
async function generateAccusePDF(demande, user) {
  try {
    ...
    doc
      ...
      .fillColor("#1e6a8e")
      .font("Helvetica-Bold")
      .text("ACCUSÉ DE RÉCEPTION DE DEMANDE", {
        align: "center",
        underline: true,
      });
    doc.moveDown(1.5);
    ...
    doc
      ...
      .text(
        `Adresse du siège (fournie par le demandeur) : ${adresseSiegeDemande}`
      )
      .text(
        `Date de dépôt : ${new Date(demande.created_at).toLocaleDateString(
          "fr-FR"
        )}`
      )
    ...
    doc
      ...
      .text(
        `Prénom du responsable : ${user.prenom_responsable || "Non renseigné"}`
      )
    ...
    doc
      .text(
        `Adresse du siège (fournie par le demandeur) : ${adresseSiegeDemandeur}`
      )
      .moveDown(1.5);
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        "Nous accusons réception de votre demande qui a été enregistrée dans nos services.",
        { align: "justify" }
      )
      .moveDown(0.5)
      .text(
        "Votre dossier sera examiné dans les meilleurs délais conformément aux procédures en vigueur.",
        { align: "justify" }
      )
      .moveDown(0.5)
      .text(
        "Vous serez informé(e) de l'évolution du traitement de votre demande via la plateforme MMIAPP.",
        { align: "justify" }
      )
      .moveDown(2);
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        "Ce document est généré automatiquement par la plateforme MMIAPP.",
        { align: "center" }
      )
      .text(
        "République Islamique de Mauritanie - Ministère des Mines et de l'Industrie",
        { align: "center" }
      );
...
app.post(
  "/api/ministre/demandes/:id/signer-et-envoyer",
  authRole([7]),
  async (req, res) => {
    ...
    try {
      console.log(
        `🔄 [Ministre] Signature et envoi automatique de la demande ${demandeId}`
      );
      ...
      console.log(
        `📄 [AUTORISATION] Génération de l'autorisation officielle signée...`
      );
      let autorisationBuffer;
      try {
        autorisationBuffer = await generateAutorisationOfficielle(
          demande,
          req.user
        );
        console.log(
          `✅ Autorisation officielle générée: ${autorisationBuffer.length} bytes`
        );
      } catch (pdfError) {
        console.error(`❌ Erreur génération autorisation: ${pdfError.message}`);
        await conn.end();
        return res.status(500).json({
          error: "Erreur lors de la génération de l'autorisation officielle",
        });
      }
      ...
      const [adminUsers] = await conn.execute(
        "SELECT id FROM utilisateurs WHERE role_id IN (1, 2, 3)"
      ); // Roles admin
      for (const adminUser of adminUsers) {
        await conn.execute(
          ...
          [
            adminUser.id,
            `Dossier ${demande.reference} clôturé et archivé. Le demandeur ${demande.prenom_responsable} ${demande.prenom_responsable} a obtenu son autorisation.`,
          ]
        );
      }
      ...
      console.log(
        `✅ [Ministre] Demande ${demande.reference} signée, autorisation générée, envoyée au demandeur et archivée avec succès`
      );
      ...
      res.json({
        success: true,
        message:
          "Autorisation officielle signée et envoyée automatiquement au demandeur. Dossier archivé et clôturé.",
...
app.get("/api/demandeur/autorisation/:id", authRole([4]), async (req, res) => {
  try {
    ...
    console.log(
      `🔍 [DEMANDEUR] Téléchargement autorisation pour demande ${id} par utilisateur ${demandeurId}`
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="autorisation-${demande.reference}.pdf"`
    );
    res.setHeader("Content-Length", demande.fichier_autorisation.length);

    console.log(
      `✅ Autorisation envoyée: ${demande.fichier_autorisation.length} bytes`
    );
    ...
    console.error(
      `❌ [DEMANDEUR] Erreur téléchargement autorisation: ${error.message}`
    );
    res.status(500).json({
...
app.get("/api/archive/demandes", authRole([1, 2, 3, 4]), async (req, res) => {
  try {
    console.log(
      `🔄 [Archive] Consultation des demandes archivées par utilisateur ${req.user.id} (rôle: ${req.user.role_id})`
    );
    ...
    console.log(
      `✅ [Archive] ${rows.length} demandes archivées récupérées pour l'utilisateur ${req.user.id}`
    );
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la consultation des archives" });
  }
});
...
app.get("/api/archive/recherche", authRole([1, 2, 3]), async (req, res) => {
  ...
  try {
    console.log(`🔍 [Archive] Recherche avec critères:`, {
      reference,
      demandeur,
      type,
      date_debut,
    ...
    if (demandeur) {
      whereClause +=
        " AND (ad.nom_responsable LIKE ? OR ad.prenom_responsable LIKE ?)";
    ...
    console.log(
      `✅ [Archive] Recherche terminée: ${rows.length} résultats trouvés`
    );
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la recherche dans les archives" });
  }
});
...
app.get("/api/archive/stats", authRole([1, 2, 3]), async (req, res) => {
  try {
    console.log(
      `📊 [Archive] Calcul des statistiques par utilisateur ${req.user.id}`
    );
    ...
    console.log(
      `✅ [Archive] Statistiques calculées: Total: ${total}, Types: ${typeStats.length}, Mois: ${monthlyStats.length}`
    );
    res.json({ stats });
  } catch (err) {
...
app.post("/api/inscription", async (req, res) => {
  ...
  if (forme_juridique === "AUTRES" && !forme_juridique_autre) {
    return res
      .status(400)
      .json({ error: "Veuillez préciser la forme juridique." });
  }
  ...
  try {
    ...
    while (idExists) {
      ...
      const [idCheck] = await conn.execute(
        "SELECT id FROM utilisateurs WHERE identifiant_unique = ?",
        [uniqueId]
      );
    ...
    const formeJuridiqueValue =
      forme_juridique === "AUTRES" ? forme_juridique_autre : forme_juridique;
    ...
    if (transporter) {
      try {
        await transporter.sendMail({
          from: `"Ministère des Mines et de l'Industrie" <${
            process.env.SMTP_USER || "diamamoudoumoussa01@gmail.com"
          }>`,
          to: email,
          subject:
      ...
      console.log(
        "Transporteur SMTP non configuré, email d'activation non envoyé"
      );
    }
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email = ? OR identifiant_unique = ?",
      [email, email]
    );
    await conn.end();
    if (rows.length === 0) {
      return res
        .status(401)
        .json({ error: "Identifiant/Email ou mot de passe incorrect." });
    }
    const user = rows[0];
...
app.post("/api/forgot-password", async (req, res) => {
  ...
  const [utilisateurs] = await conn.execute(
    "SELECT id, nom, prenom, email FROM utilisateurs WHERE email=? OR identifiant_unique=?",
    [email, email]
  );
  ...
  try {
    ...
    console.log(
      `📊 [mes-demandes] Statuts trouvés: ${[
        ...new Set(demandes.map((d) => d.statut)),
      ].join(", ")}`
    );
...
const generateReferenceMiddleware = async (req, res, next) => {
  try {
    ...
    console.log(
      `🔢 [generateReference] Date: ${date}, Séquence: ${nextSeq}, Référence: ${req.reference}`
    );
    ...
    try {
      ...
      if (typeDemande !== "boulangerie") {
        ...
        if (existing.length > 0) {
          await conn.end();
          return res.status(400).json({
            error: "Vous avez déjà une demande en cours pour ce type.",
          });
        }
      }
...
app.post("/api/verify-admin-code", async (req, res) => {
  ...
  try {
    ...
    const [existingCodes] = await conn.execute(
      "SELECT COUNT(*) as count FROM admin_access_codes"
    );
...
app.get("/api/admin/users", adminAuth(1), async (req, res) => {
  try {
    ...
    const [users] = await conn.execute(
      "SELECT id, nom, prenom, email, role_id, statut FROM utilisateurs"
    );
    // Statistiques
    ...
    users.forEach((u) => {
      ...
      switch (u.role_id) {
        case 1:
          ...
          break;
        case 2:
          roleLabel = "Secrétariat Central";
          break;
        case 3:
          roleLabel = "Secrétariat Général";
          break;
        case 4:
    ...
    try {
      ...
      switch (role_id) {
        case 1:
          ...
          break;
        case 2:
          loginPage = "/login-secretaire-central";
          break;
        case 3:
          loginPage = "/login-secretaire-general";
          break;
        case 4:
          loginPage = "/login-chef-service";
          break;
        case 5:
          loginPage = "/login-ddpi";
          break;
        case 6:
          loginPage = "/login-dgi";
          break;
        case 7:
          loginPage = "/login-commission";
          break;
        case 8:
          loginPage = "/login-comite";
          break;
        case 9:
          loginPage = "/login-ministre";
          break;
        case 10:
          loginPage = "/login";
          break;
...
app.delete("/api/admin/users/:id", adminAuth(1), async (req, res) => {
  ...
  try {
    ...
    const [demandes] = await conn.execute(
      "SELECT COUNT(*) as count FROM demandes WHERE utilisateur_id = ?",
      [userId]
    );
...
app.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [user] = await conn.execute(
        "SELECT id, nom, prenom, statut FROM utilisateurs WHERE id = ?",
        [userId]
      );
      if (user.length === 0) {
        await conn.end();
      ...
      res.json({
        success: true,
        message: `Utilisateur ${user[0].nom} ${user[0].prenom} ${
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (userData.statut !== "EN_ATTENTE") {
        ...
        return res
          .status(400)
          .json({ error: "L'utilisateur n'est pas en attente d'activation." });
      }
      ...
      try {
        ...
        console.error(
          `Erreur lors de l'envoi de l'email à ${userData.email}:`,
          emailError
        );
        res.status(500).json({
          error:
            "Erreur lors de l'envoi de l'email d'activation. Veuillez réessayer.",
        });
      }
    } catch (err) {
...
app.post("/api/login/secretaire", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res
        .status(401)
        .json({ error: "Utilisateur inconnu ou non autorisé" });
    }
    const user = rows[0];
    ...
    res
      .status(500)
      .json({ error: "Erreur serveur, contactez l'administrateur." });
  }
});
//=====Liste des demandes===================================
app.get("/api/demandes", authSecretaire, async (req, res) => {
  try {
    ...
    if (statut === "RECEPTIONNEE") {
      ...
      where =
        "d.statut IN ('DEPOSEE', 'RECEPTIONNEE', 'TRANSMISE_AU_SG', 'TRANSMISE_AU_DGI')";
    }

    console.log(
      `📋 [api/demandes] Récupération avec filtre: ${
        statut || "toutes"
      }, WHERE: ${where}`
    );
...
//===========================Accusé de réception ===============
app.post(
  "/api/demandes/:id/accuser-reception",
  authSecretaire,
  async (req, res) => {
    const demandeId = req.params.id;
    let conn;
    ...
    try {
      console.log(
        `🔄 [accuser-reception] Début du traitement pour demande ${demandeId}`
      );

      // Vérification des permissions
      if (req.user.role_id !== 2) {
        console.warn(
          `❌ [accuser-reception] Accès refusé pour l'utilisateur role_id=${req.user.role_id}`
        );
        return res.status(403).json({
          error:
            "Accès refusé : seul le secrétariat central peut générer des accusés de réception",
        });
      }
      ...
      console.log(
        `📋 [accuser-reception] Demande trouvée: ${demande.reference}, statut actuel: ${demande.statut}`
      );

      // Vérifier que la demande est au bon statut
      if (demande.statut !== "DEPOSEE") {
        console.warn(
          `❌ [accuser-reception] Statut incorrect: ${demande.statut}, attendu: DEPOSEE`
        );
        await conn.execute("SELECT RELEASE_LOCK(?)", [`accuse_${demandeId}`]);
        await conn.end();
        return res.status(400).json({
      ...
      if (demande.fichier_accuse) {
        console.warn(
          `❌ [accuser-reception] Accusé déjà existant: ${demande.fichier_accuse}`
        );
        await conn.execute("SELECT RELEASE_LOCK(?)", [`accuse_${demandeId}`]);
        await conn.end();
        return res.status(400).json({
      ...
      if (!user) {
        console.warn(
          `❌ [accuser-reception] Utilisateur ${demande.utilisateur_id} non trouvé`
        );
        await conn.execute("SELECT RELEASE_LOCK(?)", [`accuse_${demandeId}`]);
        await conn.end();
        return res
          .status(404)
          .json({ error: "Utilisateur demandeur non trouvé" });
      }

      console.log(
        `👤 [accuser-reception] Utilisateur trouvé: ${user.nom} ${user.prenom}`
      );

      // Générer le PDF d'accusé de réception
      console.log(
        `📄 [accuser-reception] Génération du PDF pour ${demande.reference}`
      );
      ...
      if (!pdfFile) {
        ...
        await conn.end();
        return res.status(500).json({
          error: "Erreur lors de la génération du PDF d'accusé de réception",
        });
      }
      ...
      try {
        ...
        if (updateResult.affectedRows === 0) {
          throw new Error(
            "La demande a été modifiée par un autre processus ou n'est plus au statut DEPOSEE"
          );
        }

        console.log(
          `✅ [accuser-reception] Statut mis à jour: DEPOSEE → RECEPTIONNEE`
        );
        ...
        const [dgiUsers] = await conn.execute(
          "SELECT id FROM utilisateurs WHERE role_id = 6"
        );
        ...
        console.log(
          `🔔 [accuser-reception] Notifications envoyées à ${dgiUsers.length} utilisateurs DGI`
        );

        // Valider la transaction
        await conn.commit();
        console.log(
          `✅ [accuser-reception] Transaction confirmée pour demande ${demandeId}`
        );
      } catch (transactionError) {
        await conn.rollback();
        console.error(
          `❌ [accuser-reception] Erreur de transaction:`,
          transactionError
        );
      ...
      res.json({
        success: true,
        message:
          "Accusé de réception généré avec succès. La DGI a été notifiée.",
      ...
      console.log(
        `🎉 [accuser-reception] Succès complet pour demande ${demande.reference}`
      );
    } catch (err) {
      ...
      if (conn) {
        try {
          ...
          console.error(
            "❌ [accuser-reception] Erreur lors du nettoyage:",
            rollbackError
          );
        }
      }
      ...
      let errorMessage =
        "Erreur interne du serveur lors de la génération de l'accusé de réception";
      let statusCode = 500;

      if (err.message.includes("modifiée par un autre processus")) {
        errorMessage =
          "La demande a été modifiée par un autre processus. Veuillez réessayer.";
        statusCode = 409;
      } else if (err.message.includes("PDF")) {
        errorMessage =
          "Erreur lors de la génération du PDF d'accusé de réception";
        statusCode = 500;
      }
...
app.get(
  "/api/demandes/:id/telecharger-accuse-secretaire",
  authSecretaire,
  async (req, res) => {
    const demandeId = req.params.id;
    ...
    try {
      ...
      doc
        ...
        .fillColor("#333")
        .text(
          "Conformément aux dispositions du décret n° 189-2009 du 07 juin 2009 relatif à l'enregistrement, au suivi et à la classification des entreprises industrielles, notamment son article 4,",
          50,
          doc.y
        );
      ...
      doc.font("Helvetica").fillColor("#333").text("pour une", 50, doc.y);
      ...
      doc.font("Helvetica-Bold").fillColor("#1890ff").text(activite, 50, doc.y);

      doc.font("Helvetica").fillColor("#333").text(",", 50, doc.y);

      doc.moveDown(1);

      // Adresse du siège dynamique
      const adresseSiege =
        demande.adresse_siege || demande.adresse || "Adresse non renseignée";
      doc
        .font("Helvetica-Bold")
        .fillColor("#1890ff")
      ...
      doc
        ...
        .fillColor("#333")
        .text(
          "Cet enregistrement est valable pour une durée de six mois, expirant le",
          50,
          doc.y
        );
      ...
      doc
        ...
        .fillColor("#333")
        .text(
          "son renouvellement est subordonné à la communication par l'entreprise de toutes les informations demandées (cf. article 5 du même décret).",
          50,
          doc.y
        );
      ...
      doc
        ...
        .fillColor("#333")
        .text(
          "Nous confirmons la réception de votre demande d'autorisation d'exploitation minière.",
          50,
          doc.y
        );
      ...
      doc
        ...
        .fillColor("#333")
        .text(
          "Votre dossier a été enregistré et sera traité dans les plus brefs délais.",
          50,
          doc.y
        );
      ...
      doc
        ...
        .fillColor("#333")
        .text(
          "Vous recevrez une notification dès que votre demande sera transmise au Secrétariat Général.",
          50,
          doc.y
        );
      ...
      doc
        ...
        .fillColor("#666")
        .text(
          "Ce document est généré automatiquement par le système MMIAPP",
          50,
          750,
          { align: "center" }
        );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="accuse_reception_${demande.reference}_nouveau_tampon.pdf"`
      );
      ...
      console.error(
        "Erreur lors de la génération de l'accusé avec nouveau tampon:",
        err
      );
      res
        .status(500)
        .json({ error: "Erreur serveur lors de la génération de l'accusé" });
    }
  }
);
...
app.get("/api/demandes/:id/document-enregistrement", async (req, res) => {
  ...
  try {
    ...
    if (!demande) {
      ...
      return res
        .status(404)
        .json({ error: "Demande non trouvée ou accès refusé" });
    }
    ...
    const adresseSiege =
      demande.adresse_siege || demande.adresse || "Adresse non renseignée";
    doc.font("Helvetica-Bold").fillColor("#1890ff").text(`**${adresseSiege}**`);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="document_enregistrement_${demande.reference}.pdf"`
    );
    ...
    console.error(
      "Erreur lors de la génération du document d'enregistrement:",
      err
    );
    res
      .status(500)
...
app.get("/api/demandes/:id/telecharger-accuse", async (req, res) => {
  ...
  try {
    ...
    if (!demande.fichier_accuse) {
      await conn.end();
      return res.status(404).json({
        error: "Aucun accusé de réception disponible pour cette demande",
      });
    }
    ...
    if (!fs.existsSync(filePath)) {
      return res
        .status(404)
        .json({ error: "Fichier d'accusé de réception introuvable" });
    }
...
app.post(
  "/api/demandes/:id/transmettre-sg",
  authSecretaire,
  async (req, res) => {
    const demandeId = req.params.id;
    let conn;
    try {
      console.log(
        `🔄 [transmettre-sg] Début du traitement pour demande ${demandeId}`
      );
      ...
      console.log(
        `📋 [transmettre-sg] Demande trouvée: ${demande.reference}, statut actuel: ${demande.statut}`
      );

      // 🔒 VÉRIFICATION : L'accusé de réception doit être fait AVANT la transmission au SG
      if (!demande.fichier_accuse) {
        console.warn(
          `❌ [transmettre-sg] Accusé de réception non généré pour la demande ${demandeId}`
        );
        await conn.end();
        return res.status(400).json({
          error:
            "Impossible de transmettre au SG. L'accusé de réception doit être généré en premier.",
        });
      }
      ...
      if (!["DEPOSEE", "RECEPTIONNEE"].includes(demande.statut)) {
        ...
        await conn.end();
        return res.status(400).json({
          error: `Impossible de transmettre. Statut actuel: ${demande.statut}`,
        });
      }
      ...
      console.log(
        `✅ [transmettre-sg] Statut mis à jour: ${statutPrecedent} → TRANSMISE_AU_SG`
      );
      ...
      await enregistrerSuivi(
        ...
        req.user.id,
        "TRANSMISSION_SG",
        `Transmise au Secrétaire Général`,
        statutPrecedent,
        "TRANSMISE_AU_SG"
      );
      ...
      console.log(
        `🔔 [transmettre-sg] Notifications envoyées à ${dgiUsers.length} utilisateurs DGI`
      );
      ...
      res.json({
        success: true,
        message:
          "Demande transmise au Secrétaire Général avec succès. La DGI a été notifiée.",
...
app.post(
  "/api/demandes/:id/transmettre-dgi",
  authSecretaire,
  async (req, res) => {
    const demandeId = req.params.id;
    let conn;
    try {
      console.log(
        `🔄 [transmettre-dgi] Début du traitement pour demande ${demandeId}`
      );
      ...
      console.log(
        `📋 [transmettre-dgi] Demande trouvée: ${demande.reference}, statut actuel: ${demande.statut}`
      );

      // Vérifier que la demande peut être transmise
      if (!["DEPOSEE", "RECEPTIONNEE"].includes(demande.statut)) {
        console.warn(
          `❌ [transmettre-dgi] Statut incorrect: ${demande.statut}`
        );
        await conn.end();
        return res.status(400).json({
          error: `Impossible de transmettre. Statut actuel: ${demande.statut}`,
      ...
      console.log(
        `✅ [transmettre-dgi] Statut mis à jour: ${statutPrecedent} → TRANSMISE_AU_DGI`
      );
      ...
      await enregistrerSuivi(
        ...
        req.user.id,
        "TRANSMISSION_DGI",
        `Transmise à la Direction Générale de l'Industrie`,
        statutPrecedent,
        "TRANSMISE_AU_DGI"
      );
...
app.post("/api/login/secretaire-general", async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email=? AND role_id=3 LIMIT 1",
      [email]
    );
    await conn.end();
    if (rows.length === 0)
...
app.post(
  "/api/demande/:id/annoter",
  authSecretaireGeneral,
  async (req, res) => {
    const { id } = req.params;
    const { commentaire } = req.body;
...
app.post("/api/login/ddpi", async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email = ? AND role_id = 5 LIMIT 1",
      [email]
    );
    await conn.end();
    if (rows.length === 0)
...
app.post("/api/login/dgi", async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email = ? AND role_id = 6 LIMIT 1",
      [email]
    );
    await conn.end();
    if (rows.length === 0)
...
app.get("/api/ddpi/stats", authDDPI, async (req, res) => {
  try {
    console.log(
      `🔄 [DDPI] Calcul des statistiques pour utilisateur ${req.user.id}`
    );
...
app.get("/api/ddpi/notifications", authDDPI, async (req, res) => {
  try {
    ...
    await conn.end();
    res.json({
      notifications: rows.map((n) => ({ ...n, isNew: n.isNew === 0 })),
    });
  } catch (err) {
    console.error(err);
...
app.get("/api/ddpi/demandes", authDDPI, async (req, res) => {
  try {
    console.log(
      `🔄 [DDPI] Récupération des demandes pour utilisateur ${req.user.id}`
    );
    ...
    console.error(
      "❌ [DDPI] Erreur lors de la récupération des demandes:",
      err
    );
...
app.post("/api/ddpi/demandes/:id/commentaire", authDDPI, async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [demandeResult] = await conn.execute(
      "SELECT statut, donnees FROM demandes WHERE id=?",
      [demandeId]
    );
    if (demandeResult.length === 0) {
      await conn.end();
...
app.post("/api/ddpi/demandes/:id/valider", authDDPI, async (req, res) => {
  ...
  try {
    ...
    await enregistrerSuivi(
      ...
      req.user.id,
      "VALIDATION_DDPI",
      commentaire || "Demande validée par DDPI",
      oldStatus,
      "VALIDEE_DDPI"
    );
    await conn.end();
...
app.post("/api/ddpi/demandes/:id/transmettre", authDDPI, async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT statut, reference FROM demandes WHERE id=?",
      [demandeId]
    );
    if (rows.length === 0) {
      await conn.end();
    ...
    if (rows[0].statut !== "VALIDEE_DDPI") {
      await conn.end();
      return res.status(400).json({ error: "Demande non validée" });
    }
    ...
    for (const dgiUser of dgiUsers) {
      await conn.execute(
        ...
        [
          dgiUser.id,
          `Nouvelle demande ${reference} transmise par le DDPI et disponible pour traitement`,
        ]
      );
    }
    console.log(
      `🔔 [transmettre-dgi] Notifications envoyées à ${dgiUsers.length} utilisateurs DGI`
    );
...
app.post("/api/dgi/demandes/:id/affecter-ddpi", authDGI, async (req, res) => {
  ...
  try {
    console.log(
      `🔄 [DGI→DDPI] Affectation de la demande ${demandeId} au DDPI par utilisateur ${req.user.id}`
    );
    ...
    const demande = demandeResult[0];
    console.log(
      `📋 [DGI→DDPI] Demande trouvée: ${demande.reference}, statut: ${demande.statut}`
    );
    ...
    await enregistrerSuivi(
      ...
      "AFFECTATION_DDPI",
      commentaire ||
        "Demande affectée au DDPI pour instruction technique par la DGI",
      oldStatus,
      newStatus
    );
    ...
    try {
      ...
      console.warn(
        `⚠️ [DGI→DDPI] Erreur lors de l'envoi de notification:`,
        notifErr.message
      );
    ...
    console.log(
      `✅ [DGI→DDPI] Demande ${demande.reference} affectée au DDPI avec succès`
    );
    res.json({
      success: true,
...
app.get("/api/dgi/stats", authDGI, async (req, res) => {
  try {
    console.log(
      `🔄 [DGI] Calcul des statistiques pour utilisateur ${req.user.id}`
    );
...
app.get("/api/dgi/demandes", authDGI, async (req, res) => {
  try {
    console.log(
      `🔄 [DGI] Récupération des demandes pour utilisateur ${req.user.id}`
    );
...
app.post("/api/dgi/demandes/:id/commentaire", authDGI, async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "Commentaire ajouté/modifié avec succès",
    });
  } catch (err) {
    console.error(err);
...
app.post(
  "/api/dgi/demandes/:id/transmettre-ministre",
  authDGI,
  async (req, res) => {
    const demandeId = req.params.id;
    const { commentaire } = req.body;
    ...
    try {
      console.log(
        `🔄 [DGI→Ministre] Transmission de la demande ${demandeId} au Ministre`
      );
      ...
      if (!statutsAutorises.includes(demande.statut)) {
        await conn.end();
        return res.status(400).json({
          error: `Impossible de transmettre au Ministre. Statut actuel: ${
            demande.statut
          }. Statuts autorisés: ${statutsAutorises.join(", ")}`,
        });
      }
      ...
      const [ministreUsers] = await conn.execute(
        "SELECT id FROM utilisateurs WHERE role_id = 7"
      ); // Role Ministre
      for (const ministreUser of ministreUsers) {
        await conn.execute(
          ...
          [
            ministreUser.id,
            `Nouvelle demande ${demande.reference} transmise par la DGI et disponible pour signature`,
          ]
        );
      }
      console.log(
        `🔔 [DGI→Ministre] Notifications envoyées à ${ministreUsers.length} utilisateurs Ministre`
      );

      await conn.end();

      console.log(
        `✅ [DGI→Ministre] Demande ${demande.reference} transmise au Ministre avec succès`
      );
      res.json({
        success: true,
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la transmission au Ministre" });
    }
  }
);
...
app.post("/api/dgi/demandes/:id/retour", authDGI, async (req, res) => {
  ...
  if (!commentaire)
    return res
      .status(400)
      .json({ error: "Commentaire obligatoire pour retour" });
...
app.post("/api/dgi/demandes/:id/transmettre", authDGI, async (req, res) => {
  ...
  try {
    ...
    if (rows[0].statut !== "VALIDEE_DGI") {
      await conn.end();
      return res
        .status(400)
        .json({ error: "Demande non validée, transmission impossible" });
    }
    const oldStatus = rows[0].statut;
  ...
  try {
    ...
    const [rows] = await conn.execute(
      `SELECT action, message, date_action, statut_precedent, nouveau_statut
       FROM suivi_demandes WHERE demande_id = ? ORDER BY date_action ASC`,
      [demandeId]
    );
    await conn.end();
    res.json({ historique: rows });
...
app.get("/api/dgi/export/pdf", authDGI, async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="statistiques_dgi.pdf"'
    );
    doc.pipe(res);
    ...
    rows.forEach((row, i) => {
      doc
        .fontSize(10)
        .text(
          `${i + 1}. [${row.reference}] ${row.type} | ${row.secteur || "-"} | ${
            row.emplacement || "-"
          } | ${row.statut} | ${row.nom} ${row.prenom} (${row.email}) | ${
            row.created_at
          }`
        );
    });
...
app.post("/api/dgi/demandes/:id/observation", authDGI, async (req, res) => {
  ...
  if (!observation)
    return res.status(400).json({ error: "Observation requise" });
  try {
    const conn = await mysql.createConnection(dbConfig);
    await conn.execute(
...
app.post(
  "/api/demandes/:id/transmettre-ministre",
  authRole([6]),
  async (req, res) => {
    const demandeId = req.params.id;
    try {
      ...
      if (!demande)
        return res.status(404).json({ error: "Demande non trouvée" });
      await conn.execute("UPDATE demandes SET statut = ? WHERE id = ?", [
        "EN_ATTENTE_SIGNATURE_MINISTRE",
        demandeId,
      ]);
      ...
      const [[ministre]] = await conn.execute(
        "SELECT id FROM utilisateurs WHERE role_id=9 LIMIT 1"
      );
      if (ministre) {
        await conn.execute(
         ...
         VALUES (?, "TRANSMISSION", ?, 0, NOW())`,
          [
            ministre.id,
            `Une nouvelle demande vous a été transmise pour signature.`,
          ]
        );
      }
      await conn.end();
...
app.post("/api/demandes/:id/retour-sg", authRole([6]), async (req, res) => {
  ...
  try {
    ...
    if (!demande) return res.status(404).json({ error: "Demande non trouvée" });
    await conn.execute('UPDATE demandes SET statut="RETOURNEE_SG" WHERE id=?', [
      demandeId,
    ]);
    await conn.execute(
      `INSERT INTO suivi_demandes (demande_id, statut_precedent, nouveau_statut, utilisateur_id, action, message)
    ...
    const [[sg]] = await conn.execute(
      "SELECT id FROM utilisateurs WHERE role_id=3 LIMIT 1"
    );
    if (sg) {
      await conn.execute(
        `INSERT INTO notifications (utilisateur_id, type, message, lu, created_at)
...
app.post(
  "/api/demandes/:id/solliciter-avis",
  authRole([6]),
  async (req, res) => {
    const demandeId = req.params.id;
    const { commission_id, message } = req.body;
    if (!commission_id)
      return res.status(400).json({ error: "Commission obligatoire" });
    try {
      const conn = await mysql.createConnection(dbConfig);
      await conn.execute(
...
app.post("/api/dgi/demandes/:id/suspendre", authDGI, async (req, res) => {
  ...
  if (!motif)
    return res.status(400).json({ error: "Motif de suspension requis" });
  ...
  try {
    ...
    const statutPrecedent = demande.statut;
    await conn.execute(
      'UPDATE demandes SET statut = "SUSPENDUE" WHERE id = ?',
      [demandeId]
    );
    ...
    await enregistrerSuivi(
      ...
      req.user.id,
      "SUSPENSION_DGI",
      `Demande suspendue: ${motif}`,
      statutPrecedent,
      "SUSPENDUE"
    );

    // Notifier le demandeur
    const [[demandeur]] = await conn.execute(
      "SELECT utilisateur_id FROM demandes WHERE id=?",
      [demandeId]
    );
    if (demandeur) {
      await conn.execute(
        ...
        [
          demandeur.utilisateur_id,
          `Votre demande a été temporairement suspendue par la DGI: ${motif}`,
        ]
      );
    }
...
app.post("/api/dgi/demandes/:id/reprendre", authDGI, async (req, res) => {
  ...
  try {
    ...
    if (!demande)
      return res.status(404).json({ error: "Demande suspendue non trouvée" });
    ...
    const statutPrecedent =
      donnees.suspension?.statut_precedent || "EN_COURS_DGI";
    ...
    if (demandeur) {
      await conn.execute(
        ...
        [
          demandeur.utilisateur_id,
          `Votre demande a été reprise par la DGI et est de nouveau en cours de traitement.`,
        ]
      );
    }
...
app.post("/api/dgi/demandes/:id/rejeter", authDGI, async (req, res) => {
  ...
  try {
    ...
    const statutPrecedent = demande.statut;
    await conn.execute(
      'UPDATE demandes SET statut = "REJETEE_DGI" WHERE id = ?',
      [demandeId]
    );
    ...
    await conn.execute(
      ...
      [
        demande.utilisateur_id,
        `Votre demande a été rejetée par la DGI. Motif: ${motif_rejet}`,
      ]
    );
...
app.post(
  "/api/dgi/demandes/:id/demander-complement",
  authDGI,
  async (req, res) => {
    const demandeId = req.params.id;
    ...
    if (!documents_requis || documents_requis.length === 0) {
      return res
        .status(400)
        .json({ error: "Liste des documents requis nécessaire" });
    }
    ...
    try {
      ...
      const statutPrecedent = demande.statut;
      await conn.execute(
        'UPDATE demandes SET statut = "COMPLEMENT_REQUIS" WHERE id = ?',
        [demandeId]
      );
      ...
      await conn.execute(
        ...
        [
          JSON.stringify(documents_requis),
          delai_reponse || "15 jours",
          observations || null,
          statutPrecedent,
          demandeId,
        ]
      ...
      await enregistrerSuivi(
        ...
        demandeId,
        req.user.id,
        "COMPLEMENT_REQUIS_DGI",
        `Compléments requis: ${documents_requis.join(", ")}`,
        statutPrecedent,
        "COMPLEMENT_REQUIS"
      );
      ...
      await conn.execute(
       ...
       VALUES (?, "COMPLEMENT_REQUIS", ?, 0, NOW())`,
        [
          demande.utilisateur_id,
          `La DGI demande des compléments pour votre demande. Délai: ${
            delai_reponse || "15 jours"
          }`,
        ]
      );
      ...
      res.json({
        success: true,
        message: "Demande de complément envoyée avec succès",
      });
    } catch (err) {
      console.error(err);
      res
        .status(500)
        .json({ error: "Erreur lors de la demande de complément" });
    }
  }
);
...
app.post(
  "/api/dgi/demandes/:id/programmer-visite",
  authDGI,
  async (req, res) => {
    const demandeId = req.params.id;
    const { date_visite, heure_visite, lieu, equipe_technique, objectifs } =
      req.body;
    if (!date_visite || !lieu) {
    ...
    try {
      ...
      const statutPrecedent = demande.statut;
      await conn.execute(
        'UPDATE demandes SET statut = "VISITE_PROGRAMMEE" WHERE id = ?',
        [demandeId]
      );
      ...
      await enregistrerSuivi(
        ...
        demandeId,
        req.user.id,
        "VISITE_PROGRAMMEE_DGI",
        `Visite technique programmée le ${date_visite} à ${lieu}`,
        statutPrecedent,
        "VISITE_PROGRAMMEE"
      );
      ...
      await conn.execute(
       ...
       VALUES (?, "VISITE_PROGRAMMEE", ?, 0, NOW())`,
        [
          demande.utilisateur_id,
          `Une visite technique a été programmée pour votre demande le ${date_visite} à ${lieu}`,
        ]
      );
      ...
      res.json({
        success: true,
        message: "Visite technique programmée avec succès",
      });
    } catch (err) {
      console.error(err);
      res
        .status(500)
        .json({ error: "Erreur lors de la programmation de visite" });
    }
  }
);
...
app.post("/api/dgi/demandes/:id/rapport-visite", authDGI, async (req, res) => {
  const demandeId = req.params.id;
  const { observations, recommandations, decision, documents_rapport } =
    req.body;
  ...
  try {
    ...
    if (!donnees.visite_technique) {
      return res.status(400).json({
        error: "Aucune visite technique programmée pour cette demande",
      });
    }
    ...
    await conn.execute(
      `UPDATE demandes SET 
       ...
       WHERE id = ?`,
      [
        JSON.stringify(rapportVisite),
        decision === "FAVORABLE" ? "EN_COURS_DGI" : "VISITE_DEFAVORABLE",
        demandeId,
      ]
    );
    ...
    await enregistrerSuivi(
      ...
      req.user.id,
      "RAPPORT_VISITE_DGI",
      `Rapport de visite enregistré. Décision: ${decision || "En attente"}`,
      "VISITE_PROGRAMMEE",
      decision === "FAVORABLE" ? "EN_COURS_DGI" : "VISITE_DEFAVORABLE"
    );
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de l'enregistrement du rapport" });
  }
});
...
app.get("/api/dgi/export-rapport-pdf", authDGI, async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="rapport_dgi_${type_rapport || "general"}.pdf"`
    );
    doc.pipe(res);
    ...
    doc
      .fontSize(18)
      .text("Rapport DGI - Ministère des Mines et de l'Industrie", {
        align: "center",
      });
    doc.moveDown();
    doc
      .fontSize(12)
      .text(`Période: ${date_debut || "Toutes"} - ${date_fin || "Toutes"}`, {
        align: "center",
      });
    doc.moveDown(2);
    ...
    const stats = {
      ...
      en_cours: rows.filter(
        (r) => !["SIGNEE_MINISTRE", "REJETEE_DGI"].includes(r.statut)
      ).length,
    };
    ...
    rows.forEach((row, i) => {
      ...
      doc
        .fontSize(10)
        .text(`${i + 1}. ${row.reference} - ${row.type} - ${row.statut}`, {
          continued: true,
        });
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!documents_requis && !informations_requises) {
      return res
        .status(400)
        .json({ error: "Documents ou informations requis" });
    }
    ...
    try {
      ...
      await conn.execute(
       ...
       WHERE id = ?`,
        [
          JSON.stringify(documents_requis || []),
          JSON.stringify(informations_requises || []),
          delai_jours || 30,
          demandeId,
        ]
      );
      ...
      await enregistrerSuivi(
        ...
        `Compléments demandés${
          delai_jours ? ` (délai: ${delai_jours} jours)` : ""
        }`,
        demande.statut,
      ...
      await conn.execute(
       ...
       VALUES (?, "COMPLEMENT", ?, 0, NOW())`,
        [
          demande.utilisateur_id,
          `Des compléments ont été demandés pour votre demande. Veuillez consulter votre espace.`,
        ]
      );
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la demande de compléments" });
    }
  }
);
...
app.post(
  "/api/dgi/demandes/:id/planifier-visite",
  authDGI,
  async (req, res) => {
    const demandeId = req.params.id;
    ...
    try {
      ...
      await conn.execute(
       ...
       WHERE id = ?`,
        [
          date_visite,
          lieu,
          motif || "Visite technique de routine",
      ...
      await conn.execute(
       ...
       VALUES (?, "VISITE", ?, 0, NOW())`,
        [
          demande.utilisateur_id,
          `Une visite technique a été planifiée pour votre demande le ${date_visite} à ${lieu}.`,
        ]
      );
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la planification de la visite" });
    }
  }
);
...
app.post("/api/dgi/demandes/:id/rapport-visite", authDGI, async (req, res) => {
  ...
  if (!rapport_visite)
    return res.status(400).json({ error: "Rapport de visite requis" });

  try {
    const conn = await mysql.createConnection(dbConfig);
    const [[demande]] = await conn.execute(
      'SELECT statut FROM demandes WHERE id=? AND statut="VISITE_PROGRAMMEE"',
      [demandeId]
    );
    if (!demande)
      return res
        .status(404)
        .json({ error: "Demande avec visite programmée non trouvée" });
    ...
    else if (decision_suite === "COMPLEMENT")
      nouveauStatut = "COMPLEMENT_REQUIS";
    ...
    await enregistrerSuivi(
      ...
      req.user.id,
      "RAPPORT_VISITE_DGI",
      `Rapport de visite enregistré. Décision: ${
        decision_suite || "En cours d'analyse"
      }`,
      "VISITE_PROGRAMMEE",
      nouveauStatut
    );
...
app.post("/api/demandes/:id/cloturer", authDGI, async (req, res) => {
  ...
  try {
    ...
    await enregistrerSuivi(
      ...
      req.user.id,
      "CLOTURE",
      `Demande clôturée: ${motif_cloture || "Clôture administrative"}`,
      demande.statut,
      "CLOTUREE"
    );
...
app.post("/api/demandes/:id/relancer", authDGI, async (req, res) => {
  ...
  if (!type_relance || !message)
    return res.status(400).json({ error: "Type et message de relance requis" });
...
app.post("/api/login/chef-service", async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email=? AND role_id=4 LIMIT 1",
      [email]
    );
    await conn.end();
    if (rows.length === 0)
...
app.get("/api/chef-service/stats", authChefService, async (req, res) => {
  try {
    ...
    rows.forEach((row) => {
      if (row.statut === "REÇUE" || row.statut === "TRANSMISE_CHEF")
        stats[0].value += row.count;
...
app.get(
  "/api/chef-service/notifications",
  authChefService,
  async (req, res) => {
    try {
      const conn = await mysql.createConnection(dbConfig);
...
app.get("/api/chef-service/demandes", authChefService, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur récupération demandes Chef de Service" });
  }
});

// Historique d'une demande Chef de Service
app.get(
  "/api/chef-service/demandes/:id/historique",
  authChefService,
  async (req, res) => {
    const demandeId = req.params.id;
    try {
...
app.post(
  "/api/chef-service/demandes/:id/commentaire",
  authChefService,
  async (req, res) => {
    const demandeId = req.params.id;
    const { commentaire } = req.body;
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Erreur ajout commentaire Chef de Service" });
    }
  }
);
...
app.post(
  "/api/chef-service/demandes/:id/valider",
  authChefService,
  async (req, res) => {
    const demandeId = req.params.id;
    const { commentaire } = req.body;
...
app.post(
  "/api/chef-service/demandes/:id/retour",
  authChefService,
  async (req, res) => {
    const demandeId = req.params.id;
    const { commentaire } = req.body;
...
app.post(
  "/api/chef-service/demandes/:id/transmettre",
  authChefService,
  async (req, res) => {
    const demandeId = req.params.id;
    try {
...
app.post("/api/demandes/:id/rejeter", authRole([4, 5, 6]), async (req, res) => {
  ...
  try {
    ...
    if (!demande) return res.status(404).json({ error: "Demande non trouvée" });
    await conn.execute('UPDATE demandes SET statut="REJETEE" WHERE id=?', [
      demandeId,
    ]);
    await conn.execute(
      `INSERT INTO suivi_demandes (demande_id, statut_precedent, nouveau_statut, utilisateur_id, action, message)
...
app.post("/api/demandes/:id/complement", authRole([4, 5]), async (req, res) => {
  ...
  try {
    ...
    if (!demande) return res.status(404).json({ error: "Demande non trouvée" });
    await conn.execute(
      'UPDATE demandes SET statut="PIECES_MANQUANTES" WHERE id=?',
      [demandeId]
    );
    await conn.execute(
      `INSERT INTO suivi_demandes (demande_id, statut_precedent, nouveau_statut, utilisateur_id, action, message)
...
app.post(
  "/api/demandes/:id/retour-etape",
  authRole([4, 6, 11]),
  async (req, res) => {
    const demandeId = req.params.id;
    const { nouvelle_etape, message } = req.body;
    if (!nouvelle_etape)
      return res.status(400).json({ error: "Nouvelle étape obligatoire" });
    try {
      const conn = await mysql.createConnection(dbConfig);
      const [[demande]] = await conn.execute(
        "SELECT statut, utilisateur_id FROM demandes WHERE id=?",
      ...
      await conn.execute(
       ...
       VALUES (?, "RETOUR", ?, 0, NOW())`,
        [
          demande.utilisateur_id,
          `Votre dossier a été renvoyé à une étape précédente : ${message}`,
        ]
      );
      await conn.end();
      res.json({ success: true });
...
app.post("/api/demandes/:id/cloturer", authRole([6, 1]), async (req, res) => {
  ...
  try {
    ...
    if (!demande) return res.status(404).json({ error: "Demande non trouvée" });
    await conn.execute('UPDATE demandes SET statut="CLOTUREE" WHERE id=?', [
      demandeId,
    ]);
    await conn.execute(
      `INSERT INTO suivi_demandes (demande_id, statut_precedent, nouveau_statut, utilisateur_id, action, message)
...
app.post("/api/login/commission", async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email=? AND role_id IN (7,8) LIMIT 1",
      [email]
    );
    await conn.end();
    if (rows.length === 0)
...
app.post(
  "/api/commission/dossiers/:id/avis",
  authCommission,
  async (req, res) => {
    const dossierId = req.params.id;
    const { type_avis, observations } = req.body;
    if (!type_avis)
      return res.status(400).json({ error: "Type d'avis requis" });
    try {
      const conn = await mysql.createConnection(dbConfig);
      ...
      let newStatut =
        type_avis === "FAVORABLE"
          ? "AVIS_FAVORABLE_COMMISSION"
          : type_avis === "DEFAVORABLE"
          ? "AVIS_DEFAVORABLE_COMMISSION"
          : type_avis === "RESERVE"
          ? "RESERVE_COMMISSION"
          : "AJOURNE";
      await conn.execute("UPDATE demandes SET statut=? WHERE id=?", [
...
app.get(
  "/api/commission/dossiers/:id/historique",
  authCommission,
  async (req, res) => {
    const dossierId = req.params.id;
    try {
...
app.post("/api/login/ministere", async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email=? AND role_id=9 LIMIT 1",
      [email]
    );
    await conn.end();
    if (rows.length === 0)
...
app.post(
  "/api/ministere/dossiers/:id/preview-pdf",
  authMinistre,
  async (req, res) => {
    let conn;
    try {
      ...
      if (demandes.length === 0) {
        return res.status(404).json({
          ...
          details:
            "Vérifiez que la demande existe et a le statut TRANSMISE_AU_MINISTRE",
        });
      }
      ...
      res.setHeader(
        "Content-Disposition",
        `inline; filename="preview-${demande.reference}.pdf"`
      );
      res.setHeader("Content-Length", pdfBuffer.length);
      res.send(pdfBuffer);

      console.log(
        `   ✅ PDF de prévisualisation généré et envoyé (${pdfBuffer.length} bytes)`
      );
    } catch (error) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      doc.on("end", () => {
        if (!hasError) {
          try {
            ...
            console.log(
              `      ✅ [PDF] Génération terminée: ${buffer.length} bytes`
            );
            resolve(buffer);
          } catch (bufferError) {
            console.error(
              `      ❌ [PDF] Erreur création buffer: ${bufferError.message}`
            );
            reject(
              new Error(`Erreur création buffer PDF: ${bufferError.message}`)
            );
          }
        }
      });
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .text(
          "Objet : Autorisation d'Installation d'une unité de recyclage de plastique"
        );
      ...
      doc.text(
        "Faisant suite à votre lettre ci-dessus référencée, j'ai l'honneur de vous informer que notre Département donne son accord de principe pour l'Installation par votre société dans la wilaya de Nouakchott."
      );

      doc.moveDown(1);

      doc.text(
        "Votre société doit s'engager à respecter strictement les dispositions de la réglementation en vigueur dans notre pays relative à cette activité notamment celle concernant les normes de santé, de sécurité, de qualité et d'environnement."
      );

      doc.moveDown(1);

      doc.text(
        "En cas de non-respect par votre Société de toutes les conditions requises pour l'installation de ce genre d'industrie, le Département se réserve le droit d'annuler cette autorisation."
      );

      doc.moveDown(1);

      doc.text(
        "Nos services concernés restent à votre disposition pour vous apporter leur appui et tout l'accompagnement nécessaire à la réalisation de votre projet."
      );
      ...
      doc.text(
        "Les informations relatives à l'activité doivent également être communiquées périodiquement (tous les trois mois) aux services du Développement industriel. Un certificat d'enregistrement valable pour six mois vous sera délivré une fois que le questionnaire dûment rempli parvient aux services."
      );

      doc.moveDown(1);

      doc.text(
        "Faute de communication de cette situation durant neuf (09) mois l'entreprise est considérée arrêtée et le certificat d'enregistrement n'est pas renouvelé. (Article 5 du décret cité supra)"
      );
      ...
      console.error(
        `      ❌ [PDF] Erreur dans generatePreviewPDF: ${error.message}`
      );
      reject(new Error(`Erreur génération PDF: ${error.message}`));
    }
  });
...
async function generateAutorisationOfficielle(demande, ministre) {
  return new Promise(async (resolve, reject) => {
    try {
      console.log(
        `      📝 [AUTORISATION] Début génération autorisation officielle pour ${demande.reference}`
      );
      ...
      doc.on("data", (chunk) => {
        if (!hasError) {
          chunks.push(chunk);
          console.log(
            `      📦 [AUTORISATION] Chunk reçu: ${chunk.length} bytes`
          );
        }
      });
      ...
      doc.on("end", () => {
        if (!hasError) {
          try {
            const buffer = Buffer.concat(chunks);
            console.log(
              `      ✅ [AUTORISATION] Génération terminée: ${buffer.length} bytes`
            );
            resolve(buffer);
          } catch (bufferError) {
            console.error(
              `      ❌ [AUTORISATION] Erreur création buffer: ${bufferError.message}`
            );
            reject(
              new Error(
                `Erreur création buffer autorisation: ${bufferError.message}`
              )
            );
          }
        }
      ...
      try {
        ...
        console.log(
          `      ⚠️ [AUTORISATION] Erreur logo: ${logoError.message}`
        );
      }
      ...
      doc
        ...
        .font("Helvetica-Bold")
        .text(
          `Monsieur le Directeur Général de l'ETS Direction Générale de l'Industrie`
        );
      ...
      doc.text(
        `Faisant suite à votre lettre ci-dessus référencée, j'ai l'honneur de vous informer que notre Département donne son accord de principe pour l'Installation par votre société dans la wilaya de ${adresse}.`
      );
      ...
      doc.text(
        "Les informations relatives à l'activité doivent également être communiquées périodiquement (tous les trois mois) aux services du Développement industriel. Un certificat d'enregistrement valide pour six mois vous sera délivré une fois que le questionnaire dûment rempli parvient aux services."
      );
      ...
      try {
        ...
        if (demande.donnees) {
          const donnees = JSON.parse(demande.donnees);
          console.log(
            `      🔍 [AUTORISATION] Données trouvées: ${
              Object.keys(donnees).length
            } champs`
          );

          // Chercher tous les champs liés à la signature
          const signatureFields = Object.keys(donnees).filter(
            (key) =>
          ...
          if (signatureFields.length > 0) {
            console.log(
              `      ✍️ [AUTORISATION] Champs signature trouvés: ${signatureFields.join(
                ", "
              )}`
            );
            ...
            for (const field of signatureFields) {
              ...
              if (
                ...
                console.log(
                  `      🖼️ [AUTORISATION] Signature uploadée trouvée dans ${field}: ${value.length} caractères`
                );
                break;
              }
            }
            ...
            if (!signatureData) {
              for (const field of signatureFields) {
                ...
                if (
                  ...
                  console.log(
                    `      ✍️ [AUTORISATION] Signature électronique trouvée dans ${field}: ${value}`
                  );
                  break;
                }
              }
        ...
        if (!signatureData) {
          console.log(
            `      🔍 [AUTORISATION] Signature non trouvée dans donnees, recherche en base...`
          );
          ...
          try {
            ...
            if (signatures.length > 0) {
              ...
              const signaturePath = path.join(
                __dirname,
                "uploads",
                signature.fichier_signature
              );

              if (fs.existsSync(signaturePath)) {
                // Lire le fichier de signature
                const signatureBuffer = fs.readFileSync(signaturePath);
                signatureData = `data:image/png;base64,${signatureBuffer.toString(
                  "base64"
                )}`;
                signatureType = "upload";
                console.log(
            ...
            console.log(
              `      ⚠️ [AUTORISATION] Erreur recherche base: ${dbError.message}`
            );
            if (conn) await conn.end();
          }
        }
        ...
        console.log(
          `      ⚠️ [AUTORISATION] Erreur parsing données: ${parseError.message}`
        );
      }
      ...
      console.error(
        `      ❌ [AUTORISATION] Erreur dans generateAutorisationOfficielle: ${error.message}`
      );
      reject(
        new Error(`Erreur génération autorisation officielle: ${error.message}`)
      );
    }
  });
}
  ...
  async (req, res) => {
    ...
    try {
      ...
      await conn.execute(
       ...
       VALUES (?, ?, ?, ?, ?, ?)`,
        [
          ...
          req.user.id,
          "SIGNATURE_MINISTRE",
          "Arrêté signé et dossier archivé",
        ]
      );
      ...
      await conn.execute(
        ...
        [
          demande.utilisateur_id,
          "Votre demande a été signée et l'autorisation délivrée.",
        ]
      );
      await conn.end();
      res.json({ success: true });
...
app.post(
  "/api/ministere/signatures/upload",
  authMinistre,
  ...
  async (req, res) => {
    try {
      if (!req.file) {
        return res
          .status(400)
          .json({ error: "Aucun fichier de signature fourni" });
      }
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de l'upload de la signature" });
    }
  }
);
...
app.delete("/api/ministere/signatures/:id", authMinistre, async (req, res) => {
  try {
    ...
    await conn.execute("DELETE FROM signatures_ministre WHERE id = ?", [
      signatureId,
    ]);
    await conn.end();
  ...
  async (req, res) => {
    try {
      ...
      if (!signature) {
        ...
        return res
          .status(404)
          .json({ error: "Signature non trouvée ou inactive" });
      }
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de l'application de la signature" });
    }
  }
);
// =================== ROUTES PNME ===================
...
app.post("/api/login/pnme", async (req, res) => {
  ...
  try {
    const conn = await mysql.createConnection(dbConfig);
    const [rows] = await conn.execute(
      "SELECT * FROM utilisateurs WHERE email = ? AND role_id = 7 LIMIT 1",
      [email]
    );
    await conn.end();
...
app.get("/api/pnme/demandes", authPNME, async (req, res) => {
  try {
    ...
    if (search) {
      whereClause +=
        " AND (d.reference LIKE ? OR u.nom LIKE ? OR u.prenom LIKE ?)";
...
app.get("/api/drmne/demandes", authDRMNE, async (req, res) => {
  try {
    ...
    if (search) {
      whereClause +=
        " AND (d.reference LIKE ? OR u.nom LIKE ? OR u.prenom LIKE ? OR u.email LIKE ?)";
...
app.post(
  "/api/drmne/demandes/:id/demander-complement",
  authDRMNE,
  async (req, res) => {
    const { id } = req.params;
    const { message } = req.body || {};
    ...
    try {
      ...
      await conn.execute(
        ...
        [
          id,
          req.user.id,
          "DEMANDER_COMPLEMENT_DRMNE",
          message || "Pièces complémentaires demandées par la DRMNE",
        ]
      );
  ...
  try {
    ...
    await conn.execute(
      ...
      [
        ...
        req.user.id,
        "VALIDER_DRMNE",
        observations || "Demande validée par la DRMNE",
      ]
    );
  ...
  try {
    ...
    await conn.execute(
      ...
      [
        ...
        req.user.id,
        "REJETER_DRMNE",
        motif || "Demande rejetée par la DRMNE",
      ]
    );
...
app.post("/api/drmne/demandes/:id/transmettre", authDRMNE, async (req, res) => {
  ...
  if (!cible || !["MMI", "DGI"].includes(cible)) {
    return res
      .status(400)
      .json({ error: "Cible de transmission invalide (MMI ou DGI)" });
  }
  ...
  try {
    ...
    const nextStatut =
      cible === "DGI" ? "TRANSMISE_AU_DGI" : "TRANSMISE_AU_MINISTRE";
    ...
    await conn.execute(
      ...
      [
        ...
        req.user.id,
        "TRANSMETTRE_DRMNE",
        `Transmise vers ${cible}${observations ? ": " + observations : ""}`,
      ]
    );
...
app.post("/api/pnme/demandes/:id/valider", authPNME, async (req, res) => {
  ...
  try {
    ...
    await conn.execute(
      ...
      [
        demande.utilisateur_id,
        `Votre demande PNME a été validée. Montant accordé: ${
          montant_accorde || "Non spécifié"
        }`,
      ]
    );
    await conn.end();
...
app.post("/api/pnme/demandes/:id/rejeter", authPNME, async (req, res) => {
  ...
  try {
    ...
    await conn.execute(
      ...
      [
        demande.utilisateur_id,
        `Votre demande PNME a été rejetée. Motif: ${motif}`,
      ]
    );
    await conn.end();
...
app.post("/api/pnme/demandes/:id/complement", authPNME, async (req, res) => {
  ...
  try {
    ...
    await conn.execute(
      ...
      [
        demande.utilisateur_id,
        `Complément d'informations requis pour votre demande PNME: ${message}`,
      ]
    );
    await conn.end();
...
app.post(
  "/api/pnme/notifications/:id/marquer-lu",
  authPNME,
  async (req, res) => {
    const { id } = req.params;
    try {
...
app.get("/api/pnme/rapport", authPNME, async (req, res) => {
  try {
    ...
    if (format === "csv") {
      // Génération CSV
      const csvHeader =
        "Reference,Entreprise,Representant,Email,Statut,Date Depot,Secteur\n";
      const csvContent = demandes
        .map((d) => {
          const donnees = d.donnees ? JSON.parse(d.donnees) : {};
          return `${d.reference},"${donnees.raison_sociale || "N/A"}","${
            d.nom
          } ${d.prenom}",${d.email},${d.statut},${d.created_at},"${
            donnees.secteur_activite || "N/A"
          }"`;
        })
        .join("\n");
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="rapport_pnme.csv"'
      );
...
app.get("/api/pnme/beneficiaires", authPNME, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des bénéficiaires" });
  }
});
// =================== FIN ROUTES PNME ===================
  ...
  async (req, res) => {
    ...
    `,
      ...
      if (dossier.length === 0) {
        ...
        return res
          .status(404)
          .json({ error: "Dossier non trouvé ou déjà traité" });
      }

      const dossierData = dossier[0];

      // Générer le PDF d'autorisation
      const pdfPath = await generateAutorisationPDF(
        dossierData,
        req.user,
        signature_type,
        signature_data
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!nouveau_service || !justification) {
      return res
        .status(400)
        .json({ error: "Nouveau service et justification requis" });
    }
      ...
      try {
        await conn.execute(
          ...
          [
            ...
            req.user.id,
            "REATTRIBUTION",
            `Ré-attribuée vers ${nouveau_service}: ${justification}`,
          ]
        );
      } catch (suiviError) {
        console.log(
          "⚠️ [reattribuer] Erreur suivi_demandes, utilisation de enregistrerSuivi:",
          suiviError.message
        );
...
app.post(
  "/api/demandes/:id/retour-etape-precedente",
  authRole([4, 6, 11, 9]),
  async (req, res) => {
    const { id } = req.params;
    ...
    if (!etape_cible || !justification) {
      return res
        .status(400)
        .json({ error: "Étape cible et justification requises" });
    }
    ...
    try {
      ...
      await conn.execute(
        ...
        [
          ...
          req.user.id,
          "RETOUR_ETAPE",
          `Retour à ${etape_cible}: ${justification}`,
        ]
      );
    ...
    try {
      ...
      await conn.execute(
        ...
        [
          ...
          req.user.id,
          "RELANCE",
          `Relance ${type_relance || "automatique"}: ${
            message || "Relance automatique"
          }`,
        ]
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (demandeCheck.length === 0) {
        ...
        return res
          .status(400)
          .json({ error: "Demande non trouvée ou déjà clôturée" });
      }
      ...
      await conn.execute(
        ...
        [
          ...
          req.user.id,
          "CLOTURE",
          `Demande clôturée: ${motif_cloture || "Clôture automatique"}`,
        ]
      );
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!commission_cible || !motif) {
      return res
        .status(400)
        .json({ error: "Commission cible et motif requis" });
    }
    ...
    try {
      ...
      // Mettre à jour le statut
      const nouveauStatut =
        commission_cible === "COMMISSION_MULTISECTORIELLE"
          ? "EN_COURS_COMMISSION"
          : "EN_COURS_COMITE";
      ...
      await conn.execute(
        ...
        [
          ...
          req.user.id,
          "DEMANDE_AVIS",
          `Avis demandé à ${commission_cible}: ${motif}`,
        ]
      );
...
app.post(
  ...
  async (req, res) => {
    ...
    if (!destinataires || !type || !message) {
      return res
        .status(400)
        .json({ error: "Destinataires, type et message requis" });
    }
        ...
        await conn.execute(
          ...
          [
            ...
            donnees_supplementaires
              ? JSON.stringify(donnees_supplementaires)
              : null,
          ]
        );
