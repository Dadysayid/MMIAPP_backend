...
async function testArchiveDemandeur() {
  ...
  try {
    ...
    const [archives] = await conn.execute('SELECT COUNT(*) as total FROM archive_demandes');
    ...
    if (archives[0].total > 0) {
      ...
      sampleArchives.forEach((archive, index) => {
        ...
        console.log(`     - Responsable: ${archive.prenom_responsable} ${archive.nom_responsable}`);
        console.log(`     - Statut: ${archive.statut}`);
        console.log(`     - Date clôture: ${archive.date_cloture}`);
        console.log(`     - Fichier autorisation: ${archive.fichier_autorisation ? 'Oui' : 'Non'}`);
        console.log('');
      });
    }
    console.log('');
    ...
    console.log('4️⃣ Vérification de la relation demandes -> archive_demandes...');
    ...
    console.log(`🔍 Résultats pour user_id ${testUserId}: ${testQuery.length} archives trouvées`);
    ...
    if (testQuery.length > 0) {
      ...
      console.log(`   - Responsable: ${firstArchive.prenom_responsable} ${firstArchive.nom_responsable}`);
      console.log(`   - Statut: ${firstArchive.statut}`);
      console.log(`   - Date archivage: ${firstArchive.date_archivage}`);
      console.log(`   - Fichier autorisation: ${firstArchive.fichier_autorisation ? 'Oui' : 'Non'}`);
    }
