  ...
  try {
    ...
    try {
      ...
      if (testResult.length > 0) {
        ...
        console.log(`   - Responsable: ${first.prenom_responsable} ${first.nom_responsable}`);
        console.log(`   - Statut: ${first.statut}`);
        console.log(`   - Date: ${first.date_archivage}`);
        console.log(`   - Autorisation: ${first.fichier_autorisation ? 'Oui' : 'Non'}`);
      }
    } catch (error) {
    ...
    try {
      ...
      const [demandeurs] = await conn.execute('SELECT id, email, role_id FROM utilisateurs WHERE role_id = 4 LIMIT 1');
      if (demandeurs.length > 0) {
        console.log(`✅ Demandeur trouvé: ID=${demandeurs[0].id}, Email=${demandeurs[0].email}`);
      } else {
