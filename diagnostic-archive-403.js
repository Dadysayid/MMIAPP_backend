...
async function diagnosticArchive403() {
  ...
  try {
    ...
    if (count[0].total === 0) {
      console.log('⚠️ Aucune archive trouvée - Création d\'une archive de test...');
    ...
    if (demandesCount[0].total > 0) {
      ...
      sampleDemandes.forEach((demande, index) => {
        console.log(`   Demande ${index + 1}: ID=${demande.id}, User=${demande.user_id}, Ref=${demande.reference}, Statut=${demande.statut}`);
      });
      console.log('');
    }
    ...
    const [usersCount] = await conn.execute('SELECT COUNT(*) as total FROM utilisateurs');
    ...
    if (usersCount[0].total > 0) {
      ...
      if (sampleUsers.length > 0) {
        sampleUsers.forEach((user, index) => {
          console.log(`   User ${index + 1}: ID=${user.id}, Email=${user.email}, Role=${user.role_id}, Nom=${user.prenom_responsable} ${user.nom_responsable}`);
        });
      } else {
    ...
    try {
      ...
      if (testQuery.length > 0) {
        ...
        console.log(`   - ID: ${first.id}, Référence: ${first.reference}, Type: ${first.type}`);
      }
    } catch (queryError) {
